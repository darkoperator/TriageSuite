//! `--sync`: refresh the bundled SQLECmd map corpus from upstream GitHub into
//! the workspace `resources/sqlite-maps/` directory. The refreshed maps take
...
const RAW_BASE: &str = "https://raw.githubusercontent.com/EricZimmerman/SQLECmd/master";
const TREE_API: &str =
    "https://api.github.com/repos/EricZimmerman/SQLECmd/git/trees/master?recursive=1";
...
/// Destination dir for the corpus, resolved relative to this crate.
fn dest_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../resources/sqlite-maps")
}

/// Fetch a URL's bytes via `curl` (already used by the fixture scripts; avoids
/// pulling an HTTP crate into the binary).
...
/// Download every `.smap` path from the repo tree into the corpus dir.
    ...
    let paths: Vec<String> = json
        ...
        .map(|arr| {
            arr.iter()
                ...
                .filter(|p| p.to_ascii_lowercase().ends_with(".smap"))
                .map(|p| p.to_string())
                .collect()
        })
        .unwrap_or_default();
